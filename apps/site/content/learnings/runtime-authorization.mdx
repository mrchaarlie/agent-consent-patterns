...
- **Filter retrieval by the *user's* live permissions**, so RAG respects the same fine-grained rules as the source systems — permission mirroring extended into the vector store (Permit.io; consistent with Oso's delegated-access model in [Delegated Authority](/learnings/delegated-authority/)).
...
- An agent's attempt to call a payments API is evaluated against its machine identity's fine-grained policy; the transfer operation requires an approval step, while the balance read sails through — see [Human-in-the-Loop Oversight](/learnings/human-in-the-loop/) (Permit.io).
