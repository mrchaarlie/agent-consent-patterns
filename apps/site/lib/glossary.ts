/**
 * Shared vocabulary used across the patterns and research topics. Each term
 * carries the definition at all three reading levels; `human` is the
 * canonical copy (mirrored into the agent view by generate-agent-view.mjs,
 * which extracts this array literal by name — keep TERMS a plain static
 * literal with no imports or computed values inside it).
 */

export interface GlossaryTerm {
  /** Anchor id on the research page, rendered as #term-<slug>. */
  slug: string;
  term: string;
  caveman: string;
  human: string;
